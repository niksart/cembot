...
def authorize(bot, user, chat, args):
    ...
    db = conn.cursor()
    db.execute("SELECT id FROM authorizations WHERE authorizer=:azer AND authorizee=:azee", {"azer": authorizer, "azee": authorizee})
    if db.fetchone() != None:
...
def deauthorize(bot, user, chat, args):
    ...
    conn = sqlite3.connect(dbfile)
    db = conn.cursor()
    db.execute("SELECT id FROM authorizations WHERE authorizer=:authorizer AND authorizee=:authorizee", {"authorizer": deauthorizer, "authorizee": deauthorizee})
    if db.fetchone() == None:
        bot.sendMessage(chat["id"], "You have not already authorized this user. You can't deauthorize it.")
    else:
        db.execute("DELETE FROM authorizations WHERE authorizer=:deauthorizer AND authorizee=:deauthorizee", {"deauthorizer": deauthorizer, "deauthorizee": deauthorizee})
